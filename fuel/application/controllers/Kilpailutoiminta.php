...
class Kilpailutoiminta extends CI_Controller
{
    ...
    private function _laatispiste_form($url, $vh = true, $result = array())
    {
        ...
        $fields['kotaso'] = array(
            'type' => 'enum',
            'mode' => 'radios',
            'options' => $koulutustaso_options,
            'value' => $result['kotaso'] ?? 1,
            'wrapper_tag' => 'li',
            'after_html' => '<span class="form-comment">Hevosen koulutustaso ko. lajissa (jos haluttua koulutustasoa
    ...
    private function _porrastetut_laatispisteet($data = array(), $url)
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST' && !isset($id)) {
            ...
            if (!$this->vrl_helper->check_vh_syntax($input['vh'])) {
                ...
                if ($query->num_rows() == 0) {
                    $data['msg'] = $this->load->view(
                        'misc/naytaviesti',
                        array('msg_type' => 'danger', 'msg' => 'Hevosta ei ole olemassa tai siltä ei löydy ominaisuuspisteitä.'),
                        true
                    );
                } else {
                    ...
                    // Vaikuttavat lajit
                    $vaikuttavia = 0;
                    $jakaja = $jaokset[$input['jaos']]['pisteet'] ?? 0; // Käytetään ?? 0 varmistamaan, että arvo on vähintään 0, jos indeksi puuttuu

                    // 1. KRIITTINEN TARKISTUS: Jos jakaja on nolla, näytetään virhe ja OHITETAAN SILMUKKA
                    if ($jakaja == 0) {
                        $data['msg'] = $this->load->view(
                            'misc/naytaviesti',
                            array(
                                'msg_type' => 'danger',
                                'msg' => 'Hevosella ei ole ominaisuuspisteitä tässä lajissa.'
                            ),
                            true
                        );
                        // Tässä kohtaa emme suorita foreach-silmukkaa ollenkaan.
                        // Asetetaan $vaikuttavia nollaan, jotta $kerroin-laskenta etenee hallitusti.
                        $vaikuttavia = 0;
                    } else {
                        // 2. JOS JAKAJASSA ON ARVO (> 0), SUORITETAAN SILMUKKA
                        foreach ($jaokset as $id => $jaos) {

                            // Varmista, että kyseessä ei ole oma jaos
                            if ($id != $input['jaos']) {

                                // Tarkistetaan yhteiset ominaisuudet JA pisteiden suhde
                                // Huomaa: jakolaskun suorittaminen on nyt turvallista, koska $jakaja > 0
                                if (
                                    $this->_traits_in_common($jaos['ominaisuudet'], $jaokset[$input['jaos']]['ominaisuudet']) &&
                                    ($jaos['pisteet'] / $jakaja) > 0.75
                                ) {
                    ...
                    // Kerroin lasketaan sen jälkeen, kun $vaikuttavia on laskettu (tai asetettu nollaksi virheen takia)
    ...
    public function k($id = -1, $error = array())
    {
        ...
        if ($id == -1) {
            ...
            if (isset($data['url']) && strlen($data['url']) > 10) {
                $this->fuel->pages->render('misc/naytaviesti', array(
                    'msg_type' => 'info',
            ...
            if (strlen($osallistujalista) > 0) {
                ...
                foreach ($osallistujat as $rivi) {
                    ...
                    if ($tageja != 0) {
                        ...
                        $vrl_sijainti < 2 // VRL-tunnus ei saa olla alussa
    ...
    private function _result_search_form($porrastettu, $data = array())
    {
        ...
        $fields['jarj_talli'] = array(
            'type' => 'text',
            'label' => 'Järjestävä talli',
            'class' => 'form-control',
            'value' => $data['jarj_talli'] ?? "",
        ...
        //varmistetaan loput tiedot
            ...
            if (!$kisa['porrastettu'] && !$nayttelyt) {
                ...
                if (isset($etuuspisteet['pisteet'])) {
                    ...
                    if ($etuuspisteet['pisteet'] < 1 and !empty($etuuspisteet['nollattu'])) {
                        $onnollattu = true;
                        $kisa['nollattu'] = true;
                    }
                }
    ...
    private function _make_classes_form($kisa, $user)
    {
        if ($kisa['porrastettu'] == 0) {
            ...
            $this->load->library('form_builder', array('submit_value' => 'Ilmoita'));
            $fields['class_amount'] = array(
                'label' => 'Luokkien määrä',
                'type' => 'number',
                'required' => TRUE,
                'min' => 1,
                'max' => 100,
    ...
    private function _make_show_result_form($jaos_id)
    {
        ...
        if (sizeof($palkinnot) > 0) {
            ...
            foreach ($palkinnot as $palkinto) {
                $fields[$palkinto['id']] = array(
                    'label' => $palkinto['palkinto'],
                    'type' => 'textarea',
                    'cols' => 40,
                    'rows' => 5,
                    'class' => 'form-control',
